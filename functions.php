...
function ibew_local_53_scripts() {
    ...
    wp_enqueue_style('ibew-local-53-main', get_template_directory_uri() . '/assets/css/main.css', array(), '1.0.7');
    wp_enqueue_script('ibew-local-53-main', get_template_directory_uri() . '/assets/js/main.js', array(), '1.0.4', true);
}
...
/**
 * Name for the member dashboard hero: PMPro billing first name, then WP first_name, then username.
 *
...
function ibew_local_53_get_member_dashboard_greeting_first_name($user) {
    ...
    $profile_first = trim((string) $user->first_name);
    if ($profile_first !== '') {
